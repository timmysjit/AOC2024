...
void recurse(vector<long long> & operands, bool& ans, long long& test_value, long long& cur, int index){
    ...
    if (ans || index == operands.size() || cur > test_value)
        return;
    ...
    cur = stoll(to_string(cur) + to_string(operands[index]));
    recurse(operands, ans, test_value, cur, index+1);
    cur = stoll(to_string(cur).substr(0, to_string(cur).size() - to_string(operands[index]).size()));
...
int main(){
    long long bridge_repair2 = bridgeRepair2();
    cout << bridge_repair2;
    return 0;
}